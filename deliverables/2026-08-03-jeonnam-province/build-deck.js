...
/* ===== 2. 한 장 요약 ===== */
{
  ...
  const k = [
    ...
    { n: "제안", l: "국가 플랫폼 위에 「전남 축산 AX 플랫폼(CowTalk Jeonnam)」을 축산 전문영역으로 탑재.\n중복 플랫폼이 아니라 국가 플랫폼의 축산 기능을 완성하는 구조입니다.", c: INK2 },
    { n: "요청", l: "① 축산 전문영역 공식 반영 ② 실무협의체 구성\n③ 기본계획 및 타당성 조사 용역 추진 — 6개월, 3~5억원 규모(안)", c: "8A5A22" }
  ];
  k.forEach((c, i) => {
    const y = 1.45 + i * 1.30;
...
sectionSlide("Ⅳ", "무엇을 만들 것인가", "COWTALK JEONNAM — ARCHITECTURE & SERVICES", "6분. 제안의 실체. 국가 플랫폼과의 역할 구분을 먼저 못 박는다.");
{
  const s = S();
  kick(s, "Ⅳ 무엇을 만들 것인가");
...

{
  ...
  T(s, "CowTalk Jeonnam — 4계층 구조", { color: W });
  const L = [
  ...
  card(s, { x: 0.62, y: 6.72, w: 12.10, h: 0.001, fill: INK });
  s.addNotes("나열이 많으므로 항목을 읽지 말고 '4개 층'만 말한다. 핵심은 ④ 행동·실행계층 — 대부분의 플랫폼이 ③에서 끝난다는 점을 강조.");
}
...
sectionSlide("Ⅵ", "추진모델", "FOUR-STAGE IMPLEMENTATION MODEL", "6분. 실무자가 기안에 옮길 수 있는 형태로 전달한다.");
{
  ...
  const p = [
    ...
    { n: "2단계", t: "축우 중심 실증", d: "2년 · 3~5개 시군 · 30~50농가 · 2,000~5,000두\n한우 · 젖소 · 송아지 육성농가 · 공공 실증목장\n질병·번식·열스트레스·사료효율·관제 실증", c: GREEN },
...

{
  ...
  const a = [
    ...
    "데이터 소유권과 활용기준 수립", "개인정보 · 방역 · 보안 검토",
...

{
  ...
  const q = [
    ...
    { q: "특정 기업에 대한 특혜가 될 수 있지 않은가", a: "특정 제품 구매사업으로 추진하지 않습니다. 전남도가 기능·성과·데이터 표준을 정하고, 카우톡은 실증과 기술검증을 수행합니다. 개방형 API와 멀티벤더 구조를 의무화합니다." },
