...
def build_korean(path):
    ...
    add_body(doc, "1.3 구제역(FMD)·우폐역(CBPP)·럼피스킨병(LSD) 등 WOAH 신고대상 질병에 관하여, 공란우 농장 및 처리시설 소재지는 채란 시점 기준 양국 합의 청정·관리 조건을 충족한다.")
    add_note(doc, "(청정지역·청정기간·예방접종 정책은 양국 수의당국 합의로 확정.)")

    add_heading(doc, "제2조 (공란우 건강 요건)")
    add_body(doc, "2.1 공란우는 채란일에 임상적으로 건강하였고 전염성 질병 증상이 없었다.")
    add_body(doc, "2.2 공란우는 채란 전 양국 합의 기간 동안 WOAH 신고대상 질병 미발생 농장에서 사육되었다.")
    add_body(doc, "2.3 공란우는 소결핵병·소브루셀라병·소백혈병(EBL)·IBR/IPV 등에 대해 검역본부 인정 방법으로 음성 확인되었거나 청정농장·청정지역 조건으로 면제된다.")
    add_note(doc, "(검사 항목·시기는 양국 합의로 확정.)")
    add_body(doc, "2.4 채란 전후 격리·관찰·건강기록은 처리시설에 보관되며 요청 시 수입국에 제공된다.")
...
def build_english(path):
    ...
    add_body(doc, "1.3 With regard to WOAH-listed diseases (FMD, CBPP, Lumpy Skin Disease, etc.), the donor farm and processing facility meet the freedom or control conditions agreed between the two veterinary authorities at the time of collection.")
    add_note(doc, "(Free zones, free periods and vaccination policy to be finalised by bilateral agreement.)")

    add_heading(doc, "Article 2 — Health requirements for donors")
    add_body(doc, "2.1 Donors were clinically healthy and showed no signs of transmissible disease on the day of oocyte collection.")
    add_body(doc, "2.2 Donors were kept, for the agreed period prior to collection, on farms free from WOAH-listed diseases.")
    add_body(doc, "2.3 Donors tested negative, by methods recognised by APQA, for Bovine Tuberculosis, Brucellosis, Enzootic Bovine Leukosis and IBR/IPV (and others as agreed), or were exempted under free-farm / free-zone conditions.")
    add_note(doc, "(Test panel and timing to be finalised by bilateral agreement.)")
    add_body(doc, "2.4 Records of donor isolation, observation and health are retained at the facility and made available to the importing authority on request.")
