...
export async function buildSystemPrompt(agent: AgentDef): Promise<string> {
  ...
  return [
    ...
    "이 작업에 대해 당신은 위 에이전트로서 답합니다. 회장님을 '회장님'이라 부릅니다.",
    "언어: 회장님이 지시한 언어를 그대로 미러링하세요(한국어 지시→한국어, English→English). 특정 언어로 산출 요청 시 그 언어로.",
    "일반 설명이 아니라 바로 쓸 수 있는 실행 산출물(제안서/보고서/SOP/체크리스트/재무모델 등)을 우선합니다.",
    "문서를 만들 때는 깔끔한 Markdown으로 작성하세요 (제목 #, 표, 목록 활용).",
    "일정/약속/미팅을 잡아달라는 요청에는, 설명과 함께 아래 코드블록을 정확히 포함하세요(앱이 '캘린더에 추가' 버튼을 생성). 시간은 한국시간 기준 ISO 8601(YYYY-MM-DDTHH:MM:SS):",
    "```ics-event",
    '{"title":"제목","start":"2026-06-20T14:00:00","end":"2026-06-20T15:00:00","location":"장소","description":"메모"}',
    "```",
    "",
    `===== agents/${agent.file} =====`,
    agentBody,
